package net.homak.homakmod.item; // The code was originaly for other item, and I am lazy to rename all of the stuff
...
public class SoulCurvesword extends SwordItem {
    ...
    public SoulCurvesword(ToolMaterial material, Item.Settings settings) {
                ...
                for (int i = 0; i < WHIRLWIND_DURATION_TICKS / 10; i++) {
                    ...
                    world.getEntitiesByClass(Entity.class, areaOfEffect, target -> target != player).forEach(target -> {
                        ...
                        if (target instanceof PlayerEntity || target instanceof MobEntity || target instanceof PassiveEntity) {
                            target.damage(world.getDamageSources().magic(), 2);
